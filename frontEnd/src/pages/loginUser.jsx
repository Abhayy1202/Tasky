...
function LoginUser() {
  ...
  const login = async (data) => {
    ...
    try {
        ...
        const response = await axios.post("/api/v1/user/login",data); // Get the user data
        const { userData } = response.data.data;
        if (userData) {
          dispatch(authLogin({ userData, role: "user" })); // Dispatch the login action with role
          navigate("/user/getAllTasks"); // Navigate to the home page
        }
    } catch (error) {
